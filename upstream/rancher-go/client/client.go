...
type Writer interface {
	...
	AddUser(username string, password string) (string, error)
	DeleteUser(userID string) error
	GetKubeConfig(clusterID string) (string, error)
	GetUserToken(username string, password string) (string, error)
}
...
func (client defaultClient) GetProjectMembers(projectID string) ([]Member, error) {
	...
	for _, item := range memberResults.Array() {
		...
		newMember := Member{
			...
			UserID:         item.Get("userId").String(),
			ProjectID:      item.Get("projectId").String(),
...
func (client defaultClient) GetProjectID(projectID string) ([]Member, error) {
	resp, err := resty.R().
		SetAuthToken(client.token).
		Get(client.serverURL + "/v3/projects/" + projectID + "/projectroletemplatebindings")
	if err != nil {
...
func (client defaultClient) GetProjectQuotas(projectID string) (*ProjectQuotas, error) {
	...
	projectUsedLimitsResult := gjson.Get(body, "resourceQuota.usedLimit")
	...
	projectUsedLimitsResult.ForEach(func(key, value gjson.Result) bool {
		pq.Used[key.String()] = value.String()
		return true
	})
...
func (client defaultClient) DeleteProject(projectID string) error {
	logrus.Debugf("Deleting project %s", projectID)
	resp, err := resty.R().
		SetAuthToken(client.token).
		Delete(client.serverURL + "/v3/projects/" + projectID)
	if err != nil {
		return err
	}
	statusCode := resp.StatusCode()
	if statusCode != http.StatusOK {
		logrus.Errorf("Delete project failed, response: %v", string(resp.Body()[:]))
		return fmt.Errorf("delete failed, response=%d", statusCode)
	}
	return nil
}

// XXX Johan Kristiansson: Note that is not possible login using this user as a global role
// binding needs to be set (/v3/globalrolebinding).
func (client defaultClient) AddUser(username string, password string) (string, error) {
	payload := map[string]interface{}{
		"enbled":             true,
		"me":                 false,
	...
	resp, err := resty.R().
		SetAuthToken(client.token).
		SetBody(payload).
		Post(client.serverURL + "/v3/users")
	if err != nil {
		return "", err
	}
	logrus.Debugf("Add user, response: %v", string(resp.Body()[:]))
	userID := gjson.Get(string(resp.Body()[:]), "id").String()

	return userID, nil
}

func (client defaultClient) DeleteUser(userID string) error {
	logrus.Debugf("Deleting user %s", userID)
	resp, err := resty.R().
		SetAuthToken(client.token).
		Delete(client.serverURL + "/v3/users/" + userID)
	if err != nil {
		return err
	}
	statusCode := resp.StatusCode()
	if statusCode != http.StatusOK {
		logrus.Errorf("Delete user failed, response: %v", string(resp.Body()[:]))
		return fmt.Errorf("delete failed, response=%d", statusCode)
	}
	return nil
}

func (client defaultClient) GetKubeConfig(clusterID string) (string, error) {
	resp, err := resty.R().
		SetAuthToken(client.token).
		Post(client.serverURL + "/v3/clusters/" + clusterID + "/?action=generateKubeconfig")
	if err != nil {
		logrus.Errorf("Failed to query Rancher project '%s': %v", clusterID, err)
		return "", err
	}

	config := gjson.Get(string(resp.Body()[:]), "config").String()

	return config, nil
}

func (client defaultClient) GetUserToken(username string, password string) (string, error) {
	resp, err := resty.R().
		SetHeader("Content-Type", "application/json").
		SetBody(&LoginOperation{Username: username, Password: password}).
		Post(client.serverURL + "/v3-public/localProviders/local?action=login")
	if err != nil {
		logrus.Errorf("Failed to login to Rancher project")
		fmt.Println(err)
		return "", err
	}

	token := gjson.Get(string(resp.Body()[:]), "token").String()
